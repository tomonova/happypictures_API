...
where anu.Email=@email
go
create or alter procedure DeleteUnassignedTags
as
DELETE FROM TAGS
	WHERE IDTAG IN
	(
...
delete from likes where PROFILEID=@userID;
...
AND CARDID IN
(
	SELECT tc.CARDID FROM TAG_CARD tc
	JOIN TAGS t ON t.IDTAG=tc.TAGID
	JOIN CARDS c ON c.IDCARD =tc.CARDID
	WHERE CARDID IN 
		(
...
AND STORYID IN
(
	SELECT ts.STORYID FROM TAG_STORY ts
	JOIN TAGS t ON t.IDTAG=ts.TAGID
	JOIN STORIES s ON s.IDSTORY =ts.STORYID
	WHERE STORYID IN 
		(
...
where IDFORMAT in
(
	select IDFORMAT from FORMATS f 
	full outer join CARDS c on c.FORMATID =f.IDFORMAT
...
END;
go
create or alter procedure ShareStory
@userID INT,
@storyID int,
@adminAccount nvarchar(150)
as
BEGIN
	declare @adminID int
	select @adminID=IDPROFILE from PROFILES
	where EMAIL = @adminAccount

	update STORIES
	set SHARED=1, PROFILEID=@adminID
	where PROFILEID=@userID 
	and IDSTORY = @storyID
	...
	where IDIMAGE in
	(
		select IMG1ID from FORMATS
		where IDFORMAT in
		(
			select FORMATID from CARDS c
			join STORY_CARD sc on sc.CARDID=c.IDCARD
			join STORIES s on sc.STORYID = s.IDSTORY
			where s.IDSTORY=@storyID
		) and IMG1ID is not null
		union
		select IMG2ID from FORMATS
		where IDFORMAT in
		(
			select FORMATID from CARDS c
			join STORY_CARD sc on sc.CARDID=c.IDCARD
			join STORIES s on sc.STORYID = s.IDSTORY
			where s.IDSTORY=@storyID
		) and img2id is not null
		union
		select IMG3ID from FORMATS
		where IDFORMAT in
		(
			select FORMATID from CARDS c
			join STORY_CARD sc on sc.CARDID=c.IDCARD
